...
| **Internal Source & Component Basenames** | `src/renderer/src/SuperCmdWhisper.tsx`, `src/renderer/src/SuperCmdRead.tsx`, `src/main/commands.ts` (`id: 'system-supercmd-whisper'`) | Internal implementation filenames and unique command IDs are preserved to maintain stable user preferences, hotkey bindings (`settings.commandHotkeys`), and component imports. |
| **Git & Upstream Attribution** | `LICENSE`, `package.json` (`repository`), documentation | Preserves upstream open-source license attribution to the original author Shobhit Bhosure per ISC license requirements, and points the repository URL to the fork `https://github.com/MdicaL7/SuperCmd`. |
